...
yxhg5Q8AtqKCMEzNbMc128qAlYojdOVW -out decrypted.txt
  ...
  Yeah - but we just need this for decryption and your original password
 is still ultra secure and never used. So you are protected against
 dictionary-attacks, brute-force attacks and more.
This password is too long and specially designed to be secure.

  openssl enc -d -aes-256-cbc -a -in outfile.enc \
          -pass pass:`xpg genpass --pass 'mysecretpassword'
              --service 'openssl-aes' -x` -out decrypted.txt

REASON
------
Everybody wants your password - give them a pass, but its never the same
per service and not your real password. Additionally, its protected in
a way that it's not available as a dictionary pass.

FINALLY
-------

The idea or reason for this tool is simple. Passwords are insecure but no-one
wants to remember passwords for every silly site or service. So you can still
use ONE password for everything you want to use AND you can use this
script to use it with other security-tools / batch-operations
to protect your data.

To generate ultra-strong passwords for encryption-services like openssl,
openvpn, disk-encryption or other systems with 'unlimited' passwords,
use -x or --strong to get a really long password thats ULTRA SECURE
but too big for a typical website.

HOW DOES THIS WORK?
-------------------
Your password is recalculated in a very complex procedure
more than 200.000 times. The result is binary crap
and converted into human readable form. This is not a toy, you can really rely
on this tool and convert an easy password into something
thats nearly unbreakable (see -x option). No one can figure out
your primary password, because you need the pass
AND the service name. Questions like 'he might using this or that'
are now history, your pass is garbage.
...
# key = OpenSSL::PKCS5.pbkdf2_hmac_sha1("Ich bin das Kennwort".reverse, "x-itec", 210375, 32)
# puts key.unpack("H*")[0].to_i(16).to_s(32)
# puts key.unpack("H*")[0].to_i(16).base62_encode.reverse
